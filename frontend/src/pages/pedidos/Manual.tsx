...
const EVENT_TYPE = 'BAZAR' as const;
...
export default function ManualOrdersPage() {
  ...
  const [phonesByProduct, setPhonesByProduct] = useState<Record<number, string>>({});
  ...
  const loadProducts = useCallback(async () => {
    ...
    try {
      ...
      console.error('Erro ao carregar produtos do bazar:', error);
      toast({
        title: 'Erro ao carregar produtos',
        description: 'Não foi possível carregar os itens cadastrados para o bazar.',
        variant: 'destructive',
      });
    } finally {
  ...
  const loadOrders = useCallback(async () => {
    ...
    try {
      ...
      console.error('Erro ao carregar pedidos do bazar:', error);
      toast({
        title: 'Erro ao carregar pedidos',
        description: 'Não foi possível recuperar os pedidos manuais de bazar.',
        variant: 'destructive',
      });
    } finally {
  ...
  const setProductPhone = (productId: number, value: string) => {
    setPhonesByProduct((prev) => ({ ...prev, [productId]: value }));
  };
  ...
  const applyDefaultPhoneToVisibleProducts = () => {
    if (!defaultPhone.trim()) return;

    setPhonesByProduct((prev) => {
      const updated = { ...prev };
      products.forEach((product) => {
        updated[product.id] = defaultPhone;
      });
      return updated;
    });
  };
  ...
  const handleLaunchSale = async (product: Product) => {
    const rawPhone = phonesByProduct[product.id]?.trim() || defaultPhone.trim();
    const quantity = quantitiesByProduct[product.id] ?? 1;

    if (!rawPhone) {
      toast({
        title: 'Telefone obrigatório',
        description: 'Informe o telefone do cliente antes de lançar a venda.',
        variant: 'destructive',
      });
      return;
    }
    ...
    const normalizedPhone = normalizeForStorage(rawPhone);
    if (
      !normalizedPhone ||
      ...
      toast({
        ...
        description: 'Use o telefone com DDD (10 ou 11 dígitos).',
        variant: 'destructive',
      });
      return;
    }
    ...
    try {
      ...
      toast({
        ...
        description: isNew
          ? `Novo pedido criado para ${normalizedPhone}`
          : `Pedido existente atualizado com ${quantity}x ${product.code}`,
      });

      setPhonesByProduct((prev) => ({ ...prev, [product.id]: '' }));
  ...
  const handleUpdateOrder = async () => {
    ...
    try {
      ...
      console.error('Erro ao atualizar pedido manual:', error);
      toast({
  ...
  const handleDeleteOrder = async (orderId: number) => {
    ...
    try {
      ...
      console.error('Erro ao excluir pedido manual:', error);
      toast({
      ...
      <div className="p-6">
        <div className="container mx-auto space-y-6">
          <div className="flex items-center justify-between">
            <h1 className="text-3xl font-bold">Pedidos Manual - Bazar</h1>
          ...
          <Tabs defaultValue="create" className="w-full">
            ...
            <TabsContent value="create" className="space-y-6">
                ...
                <CardContent>
                  <div className="grid gap-4 md:grid-cols-4">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      />
                      <Button variant="outline" onClick={applyDefaultPhoneToVisibleProducts}>
                        Aplicar
                      </Button>
                    </div>
              ...
              <Card>
                <CardHeader>
                  <CardTitle>Produtos ativos do bazar</CardTitle>
                </CardHeader>
                      ...
                      <TableBody>
                        {loadingProducts ? (
                          ...
                          products.map((product) => {
                            const phoneValue = phonesByProduct[product.id] ?? '';
                              ...
                              <TableRow key={product.id}>
                                <TableCell className="whitespace-nowrap">
                                  <Input
                                    value={phoneValue}
                                    onChange={(event) =>
                                      setProductPhone(product.id, event.target.value)
                                    }
